const container = document.getElementsByClassName("buttons comment-button-container")

for (let item of container){
    const [button1,counter1] = item.children[0].children
    const [button2,counter2] = item.children[1].children
    button1.addEventListener('click', function () {
        commentReciever(true, button1, button2, counter1, counter2)
    })
    button2.addEventListener('click', function () {
        commentReciever(false, button2, button1, counter1, counter2)
    })
}

function commentReciever(like, button_it, button_other, counter1, counter2){
    const request = new Request('/commentlike/', {
            method: 'POST',
            body: JSON.stringify({"comment_id" : button_it.dataset.id,
                                        "like" : like})
        });
